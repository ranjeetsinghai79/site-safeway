...
export async function POST(req: NextRequest) {
  try {
    ...
    const businessName = process.env.BUSINESS_NAME || 'Husky Air'
    ...
    if (resendKey && ownerEmail) {
      const html = `
        <div style="font-family:sans-serif;max-width:560px;margin:0 auto">
          ...
          <div style="margin-top:24px;padding:16px;background:#FFF7F0;border-radius:8px;border:1px solid #F97316">
            <strong>Respond fast</strong> to maximize conversion. Call them now: <a href="tel:${body.phone}" style="color:#F97316">${body.phone}</a>
          </div>
        </div>
      `
