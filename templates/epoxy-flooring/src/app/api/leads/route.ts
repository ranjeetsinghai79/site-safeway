...
export async function POST(req: NextRequest) {
  try {
    ...
    const businessName = process.env.BUSINESS_NAME || 'IronCoat Epoxy Floors'
    const businessNiche = process.env.BUSINESS_NICHE || 'epoxy-flooring'
    ...
    if (resendKey && ownerEmail) {
      const html = `
        <div style="font-family:sans-serif;max-width:560px;margin:0 auto">
          <h2 style="color:#6366F1;margin-bottom:4px">New Lead — ${businessName}</h2>
          ...
          <table style="width:100%;border-collapse:collapse;margin-top:16px">
            ...
            <tr><td style="padding:8px 0;color:#666">Phone</td><td style="padding:8px 0;font-weight:600"><a href="tel:${body.phone}" style="color:#6366F1">${body.phone}</a></td></tr>
            ${body.email ? `<tr><td style="padding:8px 0;color:#666">Email</td><td style="padding:8px 0"><a href="mailto:${body.email}" style="color:#6366F1">${body.email}</a></td></tr>` : ''}
          ...
          <div style="margin-top:24px;padding:16px;background:#EEF0FF;border-radius:8px;border-left:4px solid #6366F1">
            <strong>Respond within 5 minutes</strong> to maximize conversion. Call them now: <a href="tel:${body.phone}" style="color:#6366F1">${body.phone}</a>
          </div>
        </div>
      `
