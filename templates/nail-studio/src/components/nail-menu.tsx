...
export function NailMenuSection({ config }: Props) {
    ...
    >
      ...
      >
        <div ref={leftRef} style={{ position: "sticky", top: "6rem" }}>
          ...
          >
            From classic manicures to intricate nail art, {business.name}
            delivers salon-quality results in a clean, relaxing environment.
            Premium gel, acrylic, and dip powder only.
          </p>
        ...
        <div ref={itemsRef}>
          {services.map((service, idx) => (
            <NailRow key={service.title} service={service} index={idx} />
          ))}
        </div>
      </div>
      ...
      <style>{`
        @media (max-width: 768px) {
          ...
          .nail-menu-grid > div:first-child {
...
function NailRow({ service, index }: { service: Service; index: number }) {
