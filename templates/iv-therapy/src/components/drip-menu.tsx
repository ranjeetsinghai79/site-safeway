...
export function DripMenuSection({ config }: Props) {
    ...
    >
      ...
      >
        <div ref={leftRef} style={{ position: "sticky", top: "6rem" }}>
          ...
          >
            Every IV drip at {business.name} is formulated by licensed medical
            providers and administered by registered nurses. 100% bioavailable
            — bypasses the digestive system for immediate cellular delivery.
          </p>
          ...
          <p style={{ marginTop: "1rem", fontSize: "0.75rem", color: "var(--brand-fg-sub)" }}>
            Walk-ins welcome · 45–60 min sessions
          </p>
        </div>

        <div ref={itemsRef}>
          {services.map((service, idx) => (
            <DripRow key={service.title} service={service} index={idx} />
          ))}
        </div>
      </div>
      ...
      <style>{`
        @media (max-width: 768px) {
          ...
          .drip-menu-grid > div:first-child {
...
function DripRow({ service, index }: { service: Service; index: number }) {
