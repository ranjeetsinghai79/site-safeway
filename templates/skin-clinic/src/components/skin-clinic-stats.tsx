...
const STATS = [
  { value: 4.9,  suffix: "★",  decimals: 1, label: "Google Rating",     sub: "187+ verified reviews" },
  { value: 187,  suffix: "+",  decimals: 0, label: "Happy Clients",     sub: "and growing" },
  { value: 8,    suffix: "+",  decimals: 0, label: "Years Experience",  sub: "in Tracy" },
  { value: 100,  suffix: "%",  decimals: 0, label: "Free Consultation", sub: "no commitment" },
]
...
function StatCounter({ value, suffix, decimals, label, sub, reduced, last }: {
  ...
  return (
    <div className="stat-item" style={{ textAlign: "center", position: "relative" }}>
      <div style={{ fontFamily: "var(--font-display)", fontSize: "clamp(2.2rem,4vw,3.5rem)", fontWeight: 700, lineHeight: 1, marginBottom: "0.4rem", background: "linear-gradient(135deg,var(--brand-grad-from),var(--brand-grad-to))", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>
        <span ref={numRef}>0{suffix}</span>
      </div>
      <div style={{ fontSize: "0.8rem", fontWeight: 600, color: "var(--brand-fg)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: "0.2rem" }}>{label}</div>
      <div style={{ fontSize: "0.7rem", color: "var(--brand-fg-muted)" }}>{sub}</div>
      {!last && <div style={{ position: "absolute", right: 0, top: "15%", bottom: "15%", width: 1, background: "rgba(232,121,160,0.12)" }} />}
    </div>
  )
}

export default function SkinClinicStats({ config: _config }: Props) {
  ...
  return (
    <section ref={sectionRef} style={{ padding: "5rem 2rem", background: "var(--brand-bg-2)" }}>
      <div style={{ maxWidth: "1280px", margin: "0 auto", borderTop: "1px solid rgba(232,121,160,0.12)", paddingTop: "4rem" }}>
