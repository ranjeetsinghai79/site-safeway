...
const PHOTOS = [
  { src: "/hero-1.jpg",    label: "Chemical Peels",    sub: "from $149" },
  { src: "/hero-2.jpg",    label: "Laser Resurfacing", sub: "from $299" },
  { src: "/hero-3.jpg",    label: "Acne Treatments",   sub: "from $125" },
  { src: "/hero-4.jpg",    label: "Microneedling RF",  sub: "from $249" },
  { src: "/hero-1.jpg",    label: "Hydrafacial",       sub: "from $159" },
  { src: "/hero-2.jpg",    label: "Skin Consultation", sub: "Free" },
]

export default function SkinClinicPhotoStrip() {
  ...
  return (
    <div style={{ background: "var(--brand-bg)", padding: "3rem 0 4rem" }}>
      <div style={{ textAlign: "center", marginBottom: "2rem" }}>
        <span style={{ fontSize: "0.65rem", fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", color: "var(--brand-accent)" }}>Our Treatments</span>
      </div>
      <div ref={rowRef} className="photo-strip-grid" style={{ maxWidth: "1400px", margin: "0 auto", padding: "0 2rem", display: "grid", gridTemplateColumns: "repeat(6,1fr)", gap: "0.75rem" }}>
        {PHOTOS.map(({ src, label, sub }) => (
          <div key={label} className="photo-item" style={{ position: "relative", overflow: "hidden", borderRadius: "0.875rem", aspectRatio: "3/4", cursor: "pointer" }}>
            ...
            <div style={{ position: "absolute", inset: 0, background: "linear-gradient(180deg, transparent 40%, rgba(15,10,20,0.9) 100%)" }} />
            <div style={{ position: "absolute", bottom: 0, left: 0, padding: "1rem 0.875rem" }}>
              <p style={{ fontFamily: "var(--font-display)", fontWeight: 600, fontSize: "0.85rem", color: "#fff", lineHeight: 1.1, marginBottom: "0.15rem" }}>{label}</p>
              <p style={{ fontSize: "0.68rem", color: "var(--brand-accent)" }}>{sub}</p>
            </div>
            <div style={{ position: "absolute", top: "0.75rem", right: "0.75rem", width: 8, height: 8, borderRadius: "50%", background: "var(--brand-accent)", boxShadow: "0 0 10px rgba(232,121,160,0.7)" }} />
          </div>
        ))}
      </div>
